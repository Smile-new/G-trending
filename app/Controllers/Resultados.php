...
use App\Models\PublicacionModel; // Keep PublicacionModel
use App\Models\CategoriaModel;
use App\Models\UsuarioModel;
use CodeIgniter\HTTP\RedirectResponse;
use CodeIgniter\HTTP\ResponseInterface;

class Resultados extends BaseController // Renamed class
{
    protected $publicacionModel; // Keep publicacionModel property
    protected $categoriaModel;
    protected $usuarioModel;

    public function __construct()
    {
        $this->publicacionModel = new PublicacionModel(); // Keep PublicacionModel instantiation
    ...
    {
        $data = [
            'page_title'    => 'Gestión de Resultados', // Updated page title
            'publicaciones' => $this->publicacionModel // Keep variable name 'publicaciones' as model is PublicacionModel
        ...
        return view('dashboard/resultados', $data); // Updated view name
    }
    ...
    {
        $data = [
            'page_title' => 'Crear Nuevo Resultado', // Updated page title
            'categorias' => $this->categoriaModel->findAll(),
            'validation' => \Config\Services::validation(),
        ];
        return view('dashboard/create_resultados', $data); // Updated view name
    }
    ...
    public function store(): RedirectResponse
    {
        ...
        if (empty($loggedInUserId)) {
            session()->setFlashdata('error', 'Debes iniciar sesión para crear un resultado.'); // Updated message
            return redirect()->to(base_url('login')); // Redirigir a login si no está logueado
        }
        ...
        if ($fileFoto && $fileFoto->isValid() && !$fileFoto->hasMoved()) {
            ...
            $imageUploadPath = FCPATH . PUBLICACIONES_IMAGE_UPLOAD_PATH; // Keep Publicaciones path
            if (!is_dir($imageUploadPath)) {
                mkdir($imageUploadPath, 0777, true);
            }
            $fileFoto->move($imageUploadPath, $newNameFoto);
            $rutaFoto = PUBLICACIONES_IMAGE_UPLOAD_PATH . '/' . $newNameFoto; // Keep Publicaciones path
        }
        ...
        if ($filePdf && $filePdf->isValid() && !$filePdf->hasMoved()) {
            ...
            $pdfUploadPath = FCPATH . PUBLICACIONES_PDF_UPLOAD_PATH; // Keep Publicaciones path
            if (!is_dir($pdfUploadPath)) {
                mkdir($pdfUploadPath, 0777, true);
            }
            $filePdf->move($pdfUploadPath, $newNamePdf);
            $rutaPdf = PUBLICACIONES_PDF_UPLOAD_PATH . '/' . $newNamePdf; // Keep Publicaciones path
        }
        ...
        if ($this->publicacionModel->insert($data)) { // Use PublicacionModel
            session()->setFlashdata('success', 'Resultado "' . esc($data['titulo']) . '" creado exitosamente.'); // Updated message
        } else {
            session()->setFlashdata('error', 'Error al crear el resultado. Inténtelo de nuevo.'); // Updated message
            log_message('error', 'PublicacionModel insert error: ' . json_encode($this->publicacionModel->errors())); // Updated log message
        }

        return redirect()->to(base_url('resultado')); // Updated redirect URL
    }
    ...
    {
        $publicacion = $this->publicacionModel->find($id); // Keep variable name 'publicacion' as model is PublicacionModel

        if (!$publicacion) {
            throw new \CodeIgniter\Exceptions\PageNotFoundException('No se pudo encontrar el resultado con ID: ' . $id); // Updated message
        }

        $data = [
            'page_title'  => 'Editar Resultado', // Updated page title
            'publicacion' => $publicacion, // Keep variable name
            'categorias'  => $this->categoriaModel->findAll(),
            'usuarios'    => $this->usuarioModel->findAll(),
            'validation'  => \Config\Services::validation(),
        ];
        return view('dashboard/update_resultados', $data); // Updated view name
    }
    ...
    {
        $publicacionExistente = $this->publicacionModel->find($id); // Keep variable name
        ...
        if ($this->publicacionModel->update($id, $data)) { // Use PublicacionModel
            session()->setFlashdata('success', 'Resultado "' . esc($data['titulo']) . '" actualizado exitosamente.'); // Updated message
        } else {
            session()->setFlashdata('error', 'Error al actualizar el resultado.'); // Updated message
            log_message('error', 'PublicacionModel update error: ' . json_encode($this->publicacionModel->errors())); // Updated log message
        }
    ...
    {
        $publicacion = $this->publicacionModel->find($id); // Keep variable name

        if (!$publicacion) {
            session()->setFlashdata('error', 'Resultado no encontrado.'); // Updated message
            return redirect()->to(base_url('resultado')); // Updated redirect URL
        }
        ...
        if ($this->publicacionModel->delete($id)) { // Use PublicacionModel
            session()->setFlashdata('success', 'Resultado "' . esc($publicacion['titulo']) . '" eliminado exitosamente.'); // Updated message
        } else {
            session()->setFlashdata('error', 'Error al eliminar el resultado.'); // Updated message
            log_message('error', 'PublicacionModel delete error: ' . json_encode($this->publicacionModel->errors())); // Updated log message
        }
