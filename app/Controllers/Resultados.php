    ...
    public function update(int $id): RedirectResponse
{
    $publicacion = $this->publicacionModel->find($id);
    if (!$publicacion) {
        throw new PageNotFoundException('No se encontró la publicación con ID: ' . $id);
    }

    // Validaciones básicas (sin archivos aquí)
    $rules = [
        'titulo'            => 'required|min_length[3]|max_length[255]',
        'descripcion'       => 'permit_empty|max_length[65535]',
        'fecha_publicacion' => 'required|valid_date',
    ...
    $messages = [
        'titulo' => [
            'required'   => 'El título es obligatorio.',
            'min_length' => 'Mínimo {param} caracteres.',
            'max_length' => 'Máximo {param} caracteres.',
        ],
        'fecha_publicacion' => [
        ...
        'categoria_id' => [
            ...
            'integer'       => 'Categoría no válida.',
            'is_not_unique' => 'Categoría no encontrada.',
        ],
        'usuario_id' => [
            'required'      => 'Usuario obligatorio.',
            'integer'       => 'Usuario no válido.',
            'is_not_unique' => 'Usuario no encontrado.',
        ]
    ];
    ...
    $fileFoto = $this->request->getFile('ruta_foto');
    $rutaFoto = $publicacion['ruta_foto'];

    if ($fileFoto && $fileFoto->isValid() && !$fileFoto->hasMoved()) {
        if (!empty($rutaFoto) && file_exists(FCPATH . $rutaFoto)) {
            unlink(FCPATH . $rutaFoto);
        }

        $newFotoName = $fileFoto->getRandomName();
        $fotoPath = FCPATH . PUBLICACIONES_IMAGE_UPLOAD_PATH;
        if (!is_dir($fotoPath)) mkdir($fotoPath, 0777, true);
        $fileFoto->move($fotoPath, $newFotoName);
        $rutaFoto = PUBLICACIONES_IMAGE_UPLOAD_PATH . '/' . $newFotoName;
    ...
    $filePdf = $this->request->getFile('ruta_pdf');
    $rutaPdf = $publicacion['ruta_pdf'];

    if ($filePdf && $filePdf->isValid() && !$filePdf->hasMoved()) {
        if (!empty($rutaPdf) && file_exists(FCPATH . $rutaPdf)) {
            unlink(FCPATH . $rutaPdf);
        }

        $newPdfName = $filePdf->getRandomName();
        $pdfPath = FCPATH . PUBLICACIONES_PDF_UPLOAD_PATH;
        if (!is_dir($pdfPath)) mkdir($pdfPath, 0777, true);
        $filePdf->move($pdfPath, $newPdfName);
        $rutaPdf = PUBLICACIONES_PDF_UPLOAD_PATH . '/' . $newPdfName;
    ...
    if ($this->publicacionModel->update($id, $data)) {
        session()->setFlashdata('success', 'Resultado actualizado exitosamente.');
        return redirect()->to(base_url('resultado'));
    } else {
        session()->setFlashdata('error', 'Error al actualizar el resultado.');
        log_message('error', 'Update error: ' . json_encode($this->publicacionModel->errors()));
        return redirect()->back()->withInput();
    }
}
